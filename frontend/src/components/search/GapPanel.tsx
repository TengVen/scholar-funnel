    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-base text-ink-muted">补充检索中，分析候选论文...</p>
        </div>
      </div>
    );
  }
        ...
        {groups.map(({ cat, meta, items }) => (
          <div key={cat}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-ink-faint">{meta.desc}</span>
  ...
  return (
    <div className="card px-5 py-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <h4 className="font-serif text-base font-semibold leading-snug flex-1 text-ink">
          {c.title}
        </h4>
        <div className="flex items-center gap-2 shrink-0">
          ...
          {c.similarity != null ? (
            <span className="text-xs text-violet-300 tabular-nums font-mono">
              语义 {c.similarity.toFixed(2)}
            </span>
          ) : (
            <span className="text-xs text-gold-light tabular-nums font-mono">
      ...
      {/* Reason */}
      {c.reason && (
        <p className="text-xs text-gold-light/70 mt-1 flex items-center gap-1">
      ...
      {/* Abstract */}
      {c.abstract && (
        <p className="mt-2 text-sm text-ink-secondary leading-relaxed line-clamp-3">
