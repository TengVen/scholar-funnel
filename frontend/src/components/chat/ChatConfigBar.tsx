    ...
    <div ref={barRef} className="relative">
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        <button
          ...
          className={`relative flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs transition-colors border ${
            annOpen
              ? "border-gold/50 bg-accent-light/20 text-gold-light"
              : "border-line text-ink-muted hover:text-gold-light hover:border-gold/30"
          }`}
        ...
        <div className="absolute bottom-full mb-2 left-0 w-[420px] max-w-[calc(100vw-3rem)] bg-paper-white border border-gold/25 rounded-xl shadow-2xl shadow-black/40 p-4 z-20">
          {openPanel === "search" && (
            <div className="space-y-3">
              <p className="text-xs font-medium text-ink-faint tracking-wide">检索参数</p>
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  <span className="text-xs text-ink-muted">起始年份</span>
                  <input
                ...
                <label className="block">
                  <span className="text-xs text-ink-muted">结束年份</span>
                  <input
                    type="number" min={1900} max={2030}
              ...
              <label className="block">
                <span className="text-xs text-ink-muted">论文类型</span>
                <select
              ...
              <label className="block">
                <span className="text-xs text-ink-muted">技术探针（可选）</span>
                <input
                  type="text"
          ...
          {openPanel === "dialog" && (
            <div className="space-y-3">
              <p className="text-xs font-medium text-ink-faint tracking-wide">对话参数</p>
              <label className="block">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-ink-muted">温度（创造性）</span>
                  <span className="text-xs text-gold-light tabular-nums">{config.dialog.temperature.toFixed(1)}</span>
                </div>
                <input
                ...
                />
                <span className="text-2xs text-ink-faint">较低 = 严谨精确，较高 = 发散灵活</span>
              </label>
            </div>
          )}
          ...
          {openPanel === "advanced" && (
            <div className="space-y-3">
              <p className="text-xs font-medium text-ink-faint tracking-wide">高级选项</p>

              <div>
                <span className="text-xs text-ink-muted">向量化 / 重排模型来源</span>
              ...
              <label className="block">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-ink-muted">召回数量 top_k</span>
                  <span className="text-xs text-gold-light tabular-nums">{config.advanced.topK}</span>
                </div>
                <input
              ...
              <label className="block">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-ink-muted">相关度阈值</span>
                  <span className="text-xs text-gold-light tabular-nums">{config.advanced.scoreThreshold.toFixed(1)}</span>
                </div>
                <input
          ...
          {openPanel === "model" && (
            <div className="space-y-3">
              <p className="text-xs font-medium text-ink-faint tracking-wide">模型（使用后台内置 API Key）</p>
              <label className="block">
                <span className="text-xs text-ink-muted">选择模型</span>
                <select
              ...
              <p className="text-2xs text-ink-faint">API Key 由后台统一提供，无需填写，仅切换模型。仅本次运行生效，重启后回到默认</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="absolute bottom-full mb-2 right-0 w-[400px] max-w-[calc(100vw-3rem)] bg-paper-white border border-gold/25 rounded-xl shadow-2xl shadow-black/40 p-4 z-20">
          <div className="flex items-center justify-between mb-3">
            ...
            {announcements.length > 0 && (
              <button onClick={markAllRead} className="text-xs text-gold-light hover:underline">
                全部标为已读
              </button>
            )}
          </div>
          {announcements.length === 0 ? (
            <p className="text-sm text-ink-faint py-10 text-center">暂无公告</p>
          ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span
                        className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-2xs font-medium shrink-0"
                        style={{ color: st.color, background: `${st.color}1f` }}
                      >
                        <LevelIcon className="w-3 h-3" />
                        {st.label}
                      </span>
                      <span className="text-base font-medium text-ink truncate">{a.title}</span>
                      <span className="ml-auto flex items-center gap-1.5 shrink-0">
                        {!read && <span className="w-1.5 h-1.5 rounded-full" style={{ background: st.color }} />}
                        <span className="text-2xs text-ink-faint tabular-nums">{a.created_at.slice(0, 10)}</span>
                      </span>
                    </div>
                    <p className="text-sm text-ink-secondary mt-1.5 whitespace-pre-wrap leading-relaxed">{a.content}</p>
                  </button>
                );
              })}
            </div>
